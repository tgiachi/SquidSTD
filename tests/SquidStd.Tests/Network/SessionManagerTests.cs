...
public class SessionManagerTests
{
    ...
    public void HandleConnected_CreatesSessionAndRaisesEvent()
    {
        ...
        var connection = new FakeNetworkConnection(sessionId: 7);
    ...
    public void TryGetSession_HitAndMiss()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        manager.HandleConnected(new FakeNetworkConnection(sessionId: 3));
    ...
    public void Sessions_ReturnsSnapshotOfAll()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        manager.HandleConnected(new FakeNetworkConnection(sessionId: 1));
        manager.HandleConnected(new FakeNetworkConnection(sessionId: 2));
    ...
    public void HandleData_KnownSession_RaisesData()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        var connection = new FakeNetworkConnection(sessionId: 5);
        manager.HandleConnected(connection);
    ...
    public void HandleData_UnknownSession_DoesNotRaise()
    {
        ...
        manager.HandleData(new FakeNetworkConnection(sessionId: 123), new byte[] { 1 });
    ...
    public void HandleDisconnected_RemovesSessionAndRaisesOnce()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        var connection = new FakeNetworkConnection(sessionId: 8);
        manager.HandleConnected(connection);
    ...
    public async Task SendAsync_KnownSession_SendsToConnection()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        var connection = new FakeNetworkConnection(sessionId: 4);
        manager.HandleConnected(connection);
    ...
    public async Task DisconnectAsync_KnownSession_ClosesConnection()
    {
        using var server = NewServer();
        using var manager = NewManager(server);
        var connection = new FakeNetworkConnection(sessionId: 6);
        manager.HandleConnected(connection);
    ...
    public async Task BroadcastAsync_SendsToAll_IsolatesFailures()
    {
        ...
        var good = new FakeNetworkConnection(sessionId: 1);
        var faulty = new FakeNetworkConnection(sessionId: 2)
        {
